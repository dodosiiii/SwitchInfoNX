...
void MtpServer::removeStorage(MtpStorage* storage) {
    ...
    for (size_t i = 0; i < mStorages.size(); i++) {
        if (mStorages[i] == storage) {
            mStorages.erase(mStorages.begin()+(int)i);
...
static int receive_file(USBMtpInterface* usb, struct mtp_file_range * mfr)
{
    ...
    lseek(mfr->fd, mfr->offset, SEEK_SET);
    mtp_xfer_done = 0;
    mtp_xfer_total = mfr->length;
    mtp_xfer_active = 1;

    do
    {
        size = usb->read((char*)buffer, 16384);
        total += size;
        write(mfr->fd, buffer, size);
        mtp_xfer_done = total;
...
MtpResponseCode MtpServer::doGetObject() {
    ...
    // Extract filename for progress
    {
        const char *slash = strrchr(pathBuf.c_str(), '/');
        if (slash && *(slash+1))
            strncpy((char*)mtp_xfer_filename, slash+1, 255);
        else
            strncpy((char*)mtp_xfer_filename, pathBuf.c_str(), 255);
        mtp_xfer_filename[255] = 0;
    }
    mtp_xfer_active = 0;
    mtp_xfer_done = 0;
    mtp_xfer_total = fileLength;
...
MtpResponseCode MtpServer::doSendObjectInfo() {
    ...
    mData.getUInt16();
    mData.getUInt32();   // association desc
  ...
  if (format == MTP_FORMAT_ASSOCIATION) {
        ...
        // Set filename for progress
        {
            const char *slash = strrchr(path.c_str(), '/');
            if (slash && *(slash+1))
                strncpy((char*)mtp_xfer_filename, slash+1, 255);
            else
                strncpy((char*)mtp_xfer_filename, path.c_str(), 255);
            mtp_xfer_filename[255] = 0;
        }
        mtp_xfer_active = 0;
        mtp_xfer_done = 0;
        mtp_xfer_total = mSendObjectFileSize;
    }
...
static void deleteRecursive(const char* path) {
    ...
    if ((size_t)pathLength >= sizeof(pathbuf) - 1) {
    ...
    while ((entry = readdir(dir))) {
        ...
        snprintf(fileSpot, pathRemaining + 1, "%s", name);
